...
Tplume  =   3000;       %   Temperature of plume               [C] 
...
nt      =   5000;        %   Number of timesteps to compute
...
for n=1:nt
    ...
    for j=2:nx-1
        for i=2:nz-1
            Tnew(i,j) = T(i,j) + sz*(T(i+1,j)+T(i-1,j)-2*T(i,j)) + sx*(T(i,j+1)+T(i,j-1)-2*T(i,j));
        end
    end
